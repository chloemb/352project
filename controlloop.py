...
def GameLoop():
    ...
    while time.time() - checkpoint_time < 5:
        for event in pygame.event.get():
            if event.type == pygame.QUIT: sys.exit()

        screen.fill(black)

        textsurface = myfont.render('Sing or whistle a low note', False, white)
        ...
        frequency = myfont.render(str(curpitch), False, white)
        screen.blit(frequency, (width/2 - frequency.get_width()/2,
                                height/2 - frequency.get_height()/2 + textsurface.get_height()))
        pygame.display.flip()
    ...
    while time.time() - checkpoint_time < 5:
        ...
        textsurface = myfont.render('Sing or whistle a high note', False, white)
        screen.blit(textsurface, (width / 2 - textsurface.get_width() / 2, height / 2 - textsurface.get_height() / 2))
...
if __name__ == '__main__':
    ...
    a = Process(target=pitchdetection.pitchdetection, args=(50, 5000))
    a.start()
